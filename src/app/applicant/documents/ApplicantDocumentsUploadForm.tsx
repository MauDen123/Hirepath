...
interface ApplicantDocumentsUploadFormProps {
  applications: {
    id: string;
    vacancy: {
...
export default function ApplicantDocumentsUploadForm({ applications, userId }: ApplicantDocumentsUploadFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!selectedApplicationId || !selectedType || !file) {
    ...
    <div className="mb-6 p-4 bg-gray-50 rounded">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            ...
            {applications.map((app) => (
              <option key={app.id} value={app.id}>
                {app.vacancy.positionTitle} (Submitted: {new Date(app.submittedAt).toLocaleDateString()})
                </option>
            ))}
          </select>
        </div>
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            disabled={uploading || !selectedApplicationId || !selectedType || !file}
